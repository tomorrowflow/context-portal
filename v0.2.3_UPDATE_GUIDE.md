...
*   **Alembic Configuration Bundled & Placed:** The `alembic/` directory and `alembic.ini` are now correctly bundled within the `context-portal-mcp` package. For Alembic to function, `alembic.ini` must be present in your **workspace root directory**. Your client IDE or manual setup should ensure this file is copied there if not automatically placed. This resolves previous issues where users had to manually initialize Alembic or configure `script_location`, making database migrations more robust and automatic for supported upgrade paths.
